...
export function SpaceExplorer() {
  ...
  const { isExploring, setIsExploring } = useSpaceExplorer()
  ...
  }, [showPrompt, isExploring, setIsExploring])
    ...
    <>
          ...
          {showPrompt && (
            <div className="bg-black/50 backdrop-blur-sm border border-cyan-400/30 rounded-lg p-6 mb-8 animate-scale-in">
              <div className="text-cyan-400 text-lg mb-2 animate-pulse">
                Press ENTER to explore space
              </div>
              <div className="text-sm text-gray-400">
                Use WASD or arrow keys to fly • ESC to return
              </div>
            </div>
          )}
      ...
      {isExploring && (
        <div className="fixed inset-0 z-50 bg-black animate-fade-in">
          ...
          {/* Spaceship Game */}
          <SpaceshipGame onExit={() => setIsExploring(false)} />
        </div>
      )}
    </>
  )
